#!/usr/bin/env bash
# M5: build bf16 square-head-512 with PAGED + SPLIT (flash-decoding) enabled and run
...
# Fails CLOSED. Usage: bash build_m4.sh [seqlen] [batch]   (from hopper/; free GPU 1-7)
set -euo pipefail
...
# NOTE: varlen + SPLIT stay ENABLED (M4 delta vs build_m2.sh: split-KV / flash_fwd_combine).
# paged KV decode also routes through the varlen path.
...
"$PY" m5_fp8_microbench.py --softcap 0
